import Link from "next/link";

export default function TermsOfUse() {
  return (
    <article className="legal-page">
      <Link href="/legal" className="back-link">← Back to Legal Hub</Link>

      <h1>Terms of Use — OSD.WTF</h1>
      <p><strong>Effective Date:</strong> October 12, 2025<br />
      <strong>Last Updated:</strong> October 12, 2025</p>

      <h2>1. Acceptance of Terms</h2>
      <p>By accessing or participating in OSD.WTF, including our website, X (Twitter) Spaces, social platforms, or any related media under the OSD brand, you agree to be bound by these Terms of Use, our <Link href="/privacy">Privacy Policy</Link>, and our <Link href="/recording-policy">Recording & Clip Policy</Link>.</p>
      <p>If you do not agree, do not use or participate in OSD.WTF or any of its affiliated channels.</p>

      <h2>2. What OSD.WTF Is</h2>
      <p>OSD.WTF is an open-format broadcast platform — a mix of live discussions, cultural commentary, and educational programming built around Bitcoin culture and creative ecosystems.</p>
      <p>We host live Spaces, interviews, and community events that are primarily conversational, unfiltered, and public.</p>

      <h2>3. Eligibility and Conduct</h2>
      <p>You must be at least 18 years old to participate or interact with OSD.WTF.</p>
      <p>By using the platform, you agree to:</p>
      <ul>
        <li>Participate respectfully.</li>
        <li>Avoid harassment, hate speech, spam, or illegal content.</li>
        <li>Not impersonate others or misrepresent your identity.</li>
        <li>Not share private keys, seeds, or any sensitive personal or financial data.</li>
      </ul>
      <p>OSD.WTF reserves the right to remove, mute, or block any participant who violates these rules or disrupts the community environment.</p>

      <h2>4. Intellectual Property</h2>
      <p><strong>OSD.WTF Content</strong><br />
      All original materials — including but not limited to logos, branding, show titles, graphics, audio segments, transcripts, and site content — are the property of OSD.WTF and protected under applicable copyright and trademark laws.</p>
      <p>You may not reuse, copy, or distribute OSD.WTF materials without prior written consent.</p>
      
      <p><strong>Participant Contributions</strong><br />
      When you speak, comment, or otherwise contribute during a Space or through our site, you retain ownership of your words and likeness.</p>
      <p>However, by participating, you grant OSD.WTF a non-exclusive, worldwide, royalty-free license (subject to your consent) to record, clip, reproduce, distribute, and publicly display your contributions for educational, archival, or promotional purposes, consistent with the <Link href="/recording-policy">Recording & Clip Policy</Link>.</p>

      <h2>5. Recording and Clip Policy</h2>
      <ul>
        <li>OSD.WTF hosts may enable or disable recording on X (Twitter) Spaces at their discretion.</li>
        <li>When recording is active, X will display a "REC" indicator visible to all participants.</li>
        <li>Even when a Space is not publicly recorded, X retains internal copies for 30–120 days for moderation.</li>
        <li>OSD.WTF may access these archives within that retention window to identify and save notable moments.</li>
        <li>Any retrieved clip will not be used or published without obtaining verbal or written consent from identifiable participants.</li>
        <li>If any participant later requests a takedown, OSD.WTF may remove or edit the content at its discretion.</li>
      </ul>

      <h2>6. Third-Party Recordings</h2>
      <p>Spaces are public discussions.</p>
      <p>OSD.WTF cannot control or prevent listeners, anonymous participants, or third-party tools from recording or redistributing live audio without our knowledge.</p>
      <p>By joining a Space, you acknowledge that other participants may record, quote, or rebroadcast what is said.</p>
      <p>OSD.WTF is not responsible for any third-party recordings, reposts, or derivative uses created outside its official channels.</p>

      <h2>7. No Financial or Legal Advice</h2>
      <p>All content produced by OSD.WTF — including live discussions, recordings, or published clips — is provided for educational and entertainment purposes only.</p>
      <p>Nothing said on OSD.WTF should be interpreted as financial, investment, or legal advice.</p>
      <p>Always verify information independently before acting or transacting.</p>

      <h2>8. Views and Opinions</h2>
      <p>Views expressed by speakers, guests, or listeners are their own and do not represent the opinions, positions, or policies of OSD.WTF, OSD (Ordinals Support Desk), or any affiliated partner.</p>
      <p>We do not endorse or guarantee the accuracy of any statements made during live or recorded discussions.</p>

      <h2>9. Limitation of Liability</h2>
      <p>To the fullest extent permitted by law, OSD.WTF, its team, affiliates, and contributors shall not be liable for any direct, indirect, incidental, consequential, or special damages arising from your participation or reliance on content discussed in Spaces or on our site.</p>
      <p>Participation in OSD.WTF broadcasts is voluntary and at your own discretion.</p>

      <h2>10. Indemnification</h2>
      <p>You agree to indemnify and hold harmless OSD.WTF, its founders, contributors, and affiliates from any claim, loss, liability, or expense (including legal fees) arising out of:</p>
      <ul>
        <li>Your participation or statements in a Space,</li>
        <li>Your use of our site or content,</li>
        <li>Or your violation of these Terms.</li>
      </ul>

      <h2>11. Third-Party Services</h2>
      <p>Our website and Spaces may include links, embeds, or integrations with external platforms (e.g., X, YouTube, Spotify).</p>
      <p>We are not responsible for the content, privacy policies, or data practices of these third-party services.</p>
      <p>Your interactions with them are governed by their respective terms.</p>

      <h2>12. Data & Privacy</h2>
      <p>Your use of OSD.WTF is also governed by our <Link href="/privacy">Privacy Policy</Link>.</p>
      <p>We collect minimal data (basic analytics, voluntary form submissions, and any information required for sponsorship or contact).</p>
      <p>We do not store private keys, wallet information, or financial data.</p>

      <h2>13. Changes to Terms</h2>
      <p>We may revise or update these Terms at any time.</p>
      <p>When we make changes, we will update the "Effective Date" at the top of this page.</p>
      <p>Continued use of the site or participation in Spaces after changes are posted constitutes acceptance of the revised Terms.</p>

      <h2>14. Governing Law</h2>
      <p>These Terms are governed by and construed in accordance with the laws of the State of Texas, USA, without regard to conflict-of-law principles.</p>
      <p>Any disputes will be resolved under Texas jurisdiction unless otherwise agreed in writing.</p>

      <h2>15. Contact</h2>
      <p>For questions, complaints, or legal notices:<br />
      📩 Email: <a href="mailto:privacy@osd.wtf">privacy@osd.wtf</a><br />
      🕊 X (Twitter): <a href="https://x.com/OSDWTF" target="_blank">@OSDWTF</a></p>

      <p style={{ marginTop: 40, fontStyle: 'italic' }}>In short: OSD.WTF is a public broadcast space built on trust, respect, and creative freedom. Join the conversation, speak your truth, and remember — it's live, it's public, and it's yours to shape responsibly.</p>
    </article>
  );
}
